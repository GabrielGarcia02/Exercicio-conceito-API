...
public class ContaBancariaController {
    ...
    @PostMapping
    public ResponseEntity<ContaBancariaResponseDTO> adicionar(@RequestBody ContaBancariaRequestDTO contaBancariaRequest){
        ContaBancariaResponseDTO contaAdicionada = contaBancariaService.adicionar(contaBancariaRequest);

        return ResponseEntity
            .status(201)
            .body(contaAdicionada);
    }

    @PutMapping("/{id}")
    public ResponseEntity<ContaBancariaResponseDTO> atualizar(@PathVariable Long id, @RequestBody ContaBancariaRequestDTO contaBancariaRequest){
        ContaBancariaResponseDTO contaAtualizada = contaBancariaService.atualizar(id, contaBancariaRequest);

        return ResponseEntity
            .status(200)
            .body(contaAtualizada);
    }
